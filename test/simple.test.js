...
client.on('READY', (ready) => console.log(`${ready.user.username} is now Ready`));
...
client.on('MESSAGE_CREATE', async (message) => {
    ...
    if (command === 'ping') {
        await new ChannelNode(client).createMessage(message.channel_id, {
            content: `<@${message.author.id}> 🏓 pong !, RAM: ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} / ${(require('os').totalmem() / 1024 / 1024).toFixed(2)} MB, PING: ${client.ping}`,
            allowed_mentions: {
